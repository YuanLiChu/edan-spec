...
`meddev:archive` 的职责：检查关卡状态 → 引导验证 → 合并 delta spec → 移动文件。
`meddev:verify` 只负责三维度验证并输出报告，**不合并 spec、不移动文件**，可独立调用。
...
确认目标 feature
  → 检查 reviewGate 状态
      ├─ verify = pending          → 引导调用 meddev:verify，完成后继续
      ├─ codeReview/securityReview = pending/failed → 引导回退 meddev:task-implement
...
```bash
find MedSpec/feature/ -maxdepth 1 -mindepth 1 -type d | sort
```
...
| reviewGate 状态 | 操作 |
|-----------------|------|
| `verify.status = pending` | 引导用户调用 `meddev:verify`，等待完成后继续 |
| `codeReview` 或 `securityReview` 为 pending/failed | 引导用户调用 `meddev:task-implement` 完成剩余关卡 |
...
mv MedSpec/feature/{name} MedSpec/archive/
```
...
- [ ] `MedSpec/specs/` 存在且 delta 已正确合并
- [ ] 冲突已检测并处理（或确认无冲突）
- [ ] feature 已移至 `MedSpec/archive/`
