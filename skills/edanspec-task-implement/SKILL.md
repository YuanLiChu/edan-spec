...
以 `EdanSpec/feature/<name>/tasks.md` 为进度事实来源，但不能只读任务清单。开始每个任务前按顺序加载：

| 输入 | 加载内容 | 作用 |
|------|----------|------|
| `tasks.md` | 当前任务、实现契约引用、允许/禁止范围、验收和依赖 | 执行边界和进度 |
| `specs/` | 当前任务关联的需求 ID 与 Scenario | 行为合同 |
| `review/detail.md` | 当前任务引用的 IC-FLOW/IC-CHANGE/IC-DATA/IC-LIFE/IC-ENG/IC-VERIFY | 实现合同 |
| `EdanSpec/context/` | 对应模块文档、业务流和公共接口；目录不存在时记录缺失 | 项目上下文与复用入口 |
| 实际源文件 | 任务列出的源文件、符号、复用锚点和测试锚点 | 当前工程事实 |

轻量 feature 没有 `review/detail.md` 时，读取 `proposal.md + design.md + specs/`，并确认任务卡已经完整携带实现合同字段。任务只写“见详细设计”、没有精确契约 ID 或修改边界时，不得开始编码，先返回 `task-plan` 修正。

### 实现契约与设计漂移门禁

编码前逐项验证：

1. `specs/` 中的需求 ID、任务契约 ID 和验收标准能够相互追踪。
2. `review/detail.md` 契约状态为“可规划”，不存在未解决的阻塞项。
3. 每个代码锚点（复用锚点、入口符号、源文件、类、方法、属性、协议字段和测试锚点）在当前基线真实存在。
4. 任务的允许修改范围覆盖所需变更，且不触碰禁止修改范围。
5. 新增对象已有复用评估和批准证据，没有绕过现有接口建立平行路径。

任一项不满足即判定为**设计漂移**：停止当前任务，报告实际代码证据与冲突，先更新上游文档（spec/design/review/detail/tasks），重新确认后再执行。禁止实现 Agent 根据相似名称猜测符号、擅自改协议或扩大范围。
...
按 `AGENTS.md` 映射表加载 `rules/` 下对应规范。spawn 子 Agent 时拼入 prompt。
...
| 可自动化 UI/设备验证 | Agent 执行 QML/UI lint、组件测试、模拟器脚本、截图或视觉回归命令并保存证据 |
| 暂不可自动化的视觉/设备/完整流程验收 | 报告操作步骤、预期结果和责任人；**视觉验收保持未完成**，等待人工确认 |

**规则**：
- 自动化验证不通过 → 调 `edanspec-debugging` 排障，不得跳过
- 有可用截图或视觉回归工具时必须执行，不得直接降级为人工验收
- 暂不可自动化的验收不阻塞其他独立增量，但对应 checkbox 和 feature 完成状态必须保持未完成
- 构建、单元测试或覆盖率通过不能替代 UI 视觉、真实设备和端到端行为验收
